...
> 状态：L1-L4 已按 2026-09-04 拍板口径重排；L5 设讨论门禁，暂不派发
>
...
| --- | --- | --- | --- | --- |
| L1 | W1-IF（W2-C 可同时启动） | IF 合入后启动 W1-A / W1-B / W1-D；A-P2 与 B-P2 后滚动启动 W1-C | W1-B-P3 → W1-X | 4 |
| L2 | W2-IF | W2-A / W2-B / W2-D；W2-C 可继续独立施工 | W2-X | 4 |
...
拆分增加的是可审查的合入点，不减少可并行的独立工作。W2-C 与主线只共享总并发名额，不阻塞 L1-L4；若它跨层未完成，优先保证主线轨且总数仍不超过 4。
...
- **PR 拆分**：P1“生命周期/失败分类”→P2“原子终止/补偿”可与 W1-A/W1-D/W2-C 并行；P3“快照迁移/恢复契约”最后合入。
...
- **启动依赖**：本次文档基线合入即可，可与 W1-IF 同时启动；它是可跨 L1-L4 施工的独立支线，不等待任何 L1-L4 代码 PR。
