...
- **2026-09-04 新增 3 条**（TODO 阶段 1，对应约束 77 目标需求与能力体系、75 候选不过滤前置、76 契约统一性）。内容以 TODO 为准，本文件不复述；务必纳入工作量评估——目标需求声明是本轨的新增契约层设计。
...
- **2026-09-04 新增 4 条**（TODO 阶段 3，约束 70/73/76/77：`capabilities` 字段与 `tags` 分离、说明书 `read` 机制、容量整数化与现算、配套测试）。内容以 TODO 为准，本文件不复述。
...
- **2026-09-04 新增 2 条**（TODO 阶段 11，约束 78：声音遮挡衰减 + 配套测试）。内容以 TODO 为准。**跨轨协调点**：本轨开工前需确认 W1-C 已在规则集补齐墙门遮挡系数，否则纯函数没有参数来源；工作量比原估的大（要处理遮挡几何与隔墙/开门/关门三种情形）。
...
- **2026-09-04 新增 5 条**（TODO 阶段 12，对应约束 71 容器嵌套禁令、72 抛弃形态与两步转移、76 销毁由插件声明、74 禁复合 operation；产物配方位置另见阶段 12 条目）。内容以 TODO 为准，本文件不复述。这 5 条是本轨的主要新增工作量，评估排期时必须计入。
